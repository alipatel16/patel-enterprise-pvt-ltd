...
import {
  SalesProvider,
  useSales,
} from "../../contexts/SalesContext/SalesContext";
...
import RecordPaymentDialog from "../../components/sales/Payment/RecordPaymentDialog";
...
const ViewInvoicePageContent = () => {
  ...
  const handleQuickPaymentClick = () => {
    if (pendingInstallments.length > 0) {
      ...
      const urgentInstallment =
        pendingInstallments.find((inst) => inst.isOverdue || inst.isDueToday) ||
  ...
  const urgentInstallmentCount = pendingInstallments.filter(
    (inst) => inst.isOverdue || inst.isDueToday
  ).length;
  ...
  return (
    <Layout title="Invoice Details" breadcrumbs={breadcrumbs}>
      ...
      >
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mb: 2 }}>
            <Chip
              ...
              color={
                currentInvoice.paymentStatus === PAYMENT_STATUS.PAID
                  ? "success"
                  : "warning"
              }
            ...
            <Chip
              ...
              color={
                currentInvoice.deliveryStatus === DELIVERY_STATUS.DELIVERED
                  ? "success"
                  : "info"
              }
              variant="outlined"
            ...
            {currentInvoice.paymentStatus === PAYMENT_STATUS.EMI && (
              <Chip
                label={`${
                  currentInvoice.emiDetails?.schedule?.filter((emi) => emi.paid)
                    ?.length || 0
                }/${
                  currentInvoice.emiDetails?.schedule?.length || 0
                } Installments Paid`}
                color="info"
                variant="outlined"
                size={isMobile ? "small" : "medium"}
                    ...
                    <Box display="flex" alignItems="center" gap={1}>
                      ...
                      {tab.badge > 0 && (
                        <Chip
                          ...
                          size="small"
                          color={
                            urgentInstallmentCount > 0 ? "error" : "warning"
                          }
        ...
        <RecordPaymentDialog
          open={Boolean(invoiceForPayment)}
