// src/pages/checklists/EditChecklistPage.js
...
const EditChecklistPage = () => {
  ...
  // Load checklist and employees on mount
  useEffect(() => {
    loadData();
  }, [id, userType]);
  ...
  const loadData = async () => {
    try {
      ...
      // Load checklist and employees in parallel
      const [checklistData, employeesData] = await Promise.all([
        loadChecklistData(),
        loadEmployees()
      ]);
  ...
  const loadChecklistData = async () => {
    try {
      const checklists = await checklistService.getChecklists(userType);
      ...
      // Parse specific date if it exists
      let parsedRecurrence = { ...checklist.recurrence };
      if (parsedRecurrence.specificDate) {
        parsedRecurrence.specificDate = new Date(parsedRecurrence.specificDate);
      }
      
      return {
        ...checklist,
        recurrence: parsedRecurrence,
        // Ensure backup arrays exist
        backupEmployees: checklist.backupEmployees || [],
        backupEmployeeNames: checklist.backupEmployeeNames || []
      };
    } catch (error) {
  ...
  const loadEmployees = async () => {
    try {
      return await checklistService.getAllEmployees(userType);
    } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await checklistService.updateChecklist(userType, id, updateData, user);
      
      setSuccess('Checklist updated successfully! Redirecting...');
      setTimeout(() => {
        navigate('/checklists');
      }, 2000);
      ...
      setError(error.message || 'Failed to update checklist. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (initialLoading) {
    return (
      <Layout title="Edit Checklist" breadcrumbs={breadcrumbs}>
              ...
              <CardContent>
                ...
                <Alert severity="info" sx={{ mb: 2 }}>
                  <Typography variant="body2">
                    <strong>Backup Assignment Logic:</strong> If primary employees don't log in, 
                    backup employees will automatically receive these assignments when they log in.
                  </Typography>
                </Alert>
