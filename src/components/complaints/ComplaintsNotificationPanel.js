// src/components/complaints/ComplaintsNotificationPanel.js - With Auto-Generation
...
/**
 * Independent Complaints Notification Panel with AUTO-GENERATION on load
 * @param {Object} props
...
const ComplaintsNotificationPanel = ({ compact = false, maxItems = 10 }) => {
  ...
  // Use ref to track if auto-generation has been triggered
  ...
  // Auto-generate notifications on mount (AUTOMATIC)
  useEffect(() => {
    const autoGenerateNotifications = async () => {
      if (!user || !userType || hasAutoGenerated.current) return;

      // Mark as generated so it doesn't run again
      hasAutoGenerated.current = true;

      try {
        console.log('🚀 AUTO-GENERATING complaint notifications on page load...');
        ...
        // Show subtle success message only if notifications were generated
        if (result.generated > 0 || result.cleanedUp > 0) {
          setActionResult({
            type: 'success',
            message: `Auto-loaded: ${result.generated} new notifications (${result.dueToday} due today, ${result.overdue} overdue)`
          });
        ...
        setActionResult({
          type: 'warning',
          message: 'Could not auto-load notifications. Click "Generate" to try again.'
        });
      } finally {
  ...
  // Generate complaint notifications (MANUAL - for user control)
            ...
            >
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                <IconButton
                  ...
                  color="secondary"
                  title="Refresh & Generate Notifications"
                >
              ...
              {complaintNotifications.length > 0 ? (
                ...
                >
                  ...
                  <Typography variant="body2" gutterBottom>
                    {autoGenerationComplete ? 'No Notifications' : 'Loading Notifications...'}
                  </Typography>
                  <Typography variant="caption" textAlign="center">
                    {autoGenerationComplete 
                      ? 'All due/overdue complaints have been checked'
                      : 'Checking for due and overdue complaints...'
                    }
                  </Typography>
                </Box>
              )}
  ...
  // Full notification panel (unchanged for standalone page)
  return (
    <Card>
      <CardContent sx={{ p: 0 }}>
        {/* Header */}
          ...
          {loading || generationLoading ? (
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', p: 4 }}>
              ...
              <Typography variant="body2">
                {generationLoading ? 'Generating notifications...' : 'Loading...'}
              </Typography>
            </Box>
          ) : complaintNotifications.length === 0 ? (
            <Box
